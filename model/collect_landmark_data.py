...
print("📸 Press a key (0-9/A-Z) to start collecting 50 samples. Press ENTER to save data. ESC to exit.")
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        # If less than 2 hands, pad with zeros to maintain 126 length (2*21*3)
        while len(landmark_list) < 126:
            landmark_list.extend([0.0, 0.0, 0.0])

        if collecting and len(landmark_list) == 126:
    ...
    if key != -1:
        if key == 27:  # ESC - exit
            break
        elif key == 13:  # ENTER - save current data (overwrite)
            if data:
                df = pd.DataFrame(data)
                df['label'] = labels
                os.makedirs("data", exist_ok=True)
                df.to_csv("data/isl_landmark_data.csv", index=False)
                print("💾 Data saved to 'data/isl_landmark_data.csv'")
                data.clear()
                labels.clear()
        else:
